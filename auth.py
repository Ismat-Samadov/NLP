...
if authentication_status:
    ...
    # if get_ip_geolocation(get_public_ip()):
    #     print("Geolocation Data:")
    #     for key, value in get_ip_geolocation(get_public_ip()).items():
    #         st.write(f"{key}: {value}")

    def main():
        if get_public_ip():
            geolocation_data = get_ip_geolocation(get_public_ip())
            if geolocation_data:
                st.sidebar.write(f"IP address: {get_public_ip()}")
